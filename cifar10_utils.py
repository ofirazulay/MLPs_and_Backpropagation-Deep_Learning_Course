...
def get_dataloader(dataset, batch_size, return_numpy=False): #(פונ' שקוראים לה מבחוץ מחזירה את שלושת סטים אימון אימות בחינה)
...
def numpy_collate_fn(batch): #(פונ' עזר שקוראים לה מבפנים)
    imgs = torch.stack([b[0] for b in batch], dim=0).numpy()
    labels = np.array([b[1] for b in batch], dtype=np.int32)
    return imgs, labels


def read_data_sets(data_dir, validation_size=5000): #(פונ' עזר שקוראים לה בפונ' האחרונה)
    """
